"""
    Adopted from AnomalyGPT: Detecting Industrial Anomalies using Large Vision-Language Models
"""
import numpy as np
import cv2
import sys
...
def patch_ex(ima_dest, ima_src=None, same=False, num_patches=1,
    ...
    for i in range(num_patches):
        if i == 0 or np.random.randint(2) > 0:  # at least one patch
            ...
            if patch_mask is not None:
                ...
                label_boxes.append([coor_min_dim2, coor_min_dim1, coor_max_dim2, coor_max_dim1])
    ...
    return patchex, label, label_boxes
